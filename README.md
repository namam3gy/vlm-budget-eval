...
자세한 분석은 [`docs/insights/insights_ko.md`](./docs/insights/insights_ko.md) (한국어) / [`docs/insights/insights.md`](./docs/insights/insights.md) (English), 인터랙티브 데모는 [`notebooks/experiment.ipynb`](./notebooks/experiment.ipynb) (37셀, 사전 실행, 한국어).
...
uv run python scripts/run_dense_sweep.py

# 5. Prompt-nudge 변형
uv run python scripts/run_nudge.py
...
```
src/vlm_budget_eval/        # 핵심 엔진 (EvalConfig + run_episode)
  budget_eval.py
scripts/                    # 실행 가능 드라이버
  ...
  run_abstention.py           # ABSTAIN 프로브 (Phase 1c)
  summarize_runs.py           # all_runs_summary.csv 롤업
  analyze_runs.py             # budget curve, modality mix, subject crosstab, bias bucket
  analyze_calibration.py      # Phase 1a calibration metrics
  analyze_difficulty.py       # Phase 1b difficulty stratification
  analyze_abstention.py       # Phase 1c abstention metrics
notebooks/experiment.ipynb  # 37셀 데모 노트북 (한국어, 사전 실행)
docs/
...
tests/                      # 테스트 (TBD)
references/
  project.md / project_ko.md   # paper-scope 연구 계획
  roadmap.md / roadmap_ko.md   # 살아 있는 진행 추적
...
다음 단계 후보는 [`docs/insights/insights_ko.md`](./docs/insights/insights_ko.md)의 *다음 단계 후보* 섹션 참고.
